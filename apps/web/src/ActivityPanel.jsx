...
const kindPresentation = {
  plan: { label: "Plan", Icon: DocumentBulletList20Regular },
  progress: { label: "Progress", Icon: Pulse20Regular },
  provider_summary: { label: "Provider summary", Icon: Bot20Regular },
  tool: { label: "Tool", Icon: Toolbox20Regular },
  web_action: { label: "Web action", Icon: Globe20Regular },
...
const eventCopy = (event) => {
  ...
  if (event.kind === "tool") return { title: payload.name, detail: payload.summary };
    ...
    <>
      ...
      <span className="activity-event-copy">
        <span className="activity-event-kind">
          {presentation.label}
          {event.provenance === "provider_generated" && <span className="activity-provider-label">Provider generated</span>}
        </span>
...
export function ActivityTimeline({ events = [], feedState = "empty", onRetry }) {
  const emptyState = events.length === 0 ? statusCopy[feedState] : undefined;
  return (
    <div className="activity-timeline-wrap">
      ...
      {emptyState ? (
        ...
        <ol className="activity-timeline" aria-label="Turn activity">
          {events.map((event) => <ActivityEventRow event={event} key={event.eventId || event.sequence} />)}
        </ol>
      )}
    </div>
  );
  ...
  return (
    <>
      ...
      >
        ...
        <p className="activity-panel-intro">Progress, tools, sources, and actions ONEComputer recorded for this turn.</p>
