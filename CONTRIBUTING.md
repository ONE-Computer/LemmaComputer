...
secrets, and explicit overrides. Ordinary defaults remain in the contract;
worktree Docker names and image tags derive from its generated installation ID.
`scripts/setup/environment-files.mjs` reads and compacts this single file while
preserving existing secrets and resource identity. `.runtime-env` files are
disposable service and Compose projections generated by the repository commands.
