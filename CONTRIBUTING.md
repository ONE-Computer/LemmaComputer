...
| `npm run env:check` | Resolved defaults, required generated values in `.env`, and strict selected-profile validation |
...
The installation keeps one compact `.env` containing setup choices, generated
secrets, optional external-integration fields, and explicit overrides. Postmark,
Microsoft and Google login, Microsoft 365, and customer SSO trusted-origin fields
remain visible when blank, alongside email transport and invitation delivery
mode. GitHub and Google Workspace MCP credentials are configured through the
product UI; their optional deployment-wide `.env` fallbacks are omitted when
empty, and existing nonempty overrides are preserved. Ordinary internal defaults
remain in the contract; worktree Docker names and image tags derive from its
generated installation ID.
`scripts/setup/environment-files.mjs` reads and compacts this single file while
preserving configured values, existing secrets, and resource identity.
`.runtime-env` files are disposable service and Compose projections generated by
