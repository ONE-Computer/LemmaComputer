...
const sections = [
  section("Deployment profile and local topology", "Set the profile explicitly for every deployment. Worktree values are replaced by npm run worktree:init.", [
    ...
    variable("LEMMACOMPUTER_INSTALLATION_ID", "", "Generated once for a local worktree; keeps derived Docker names stable. Do not change it for an existing installation.", { optional: true }),
...
const qualificationSections = [
  section("OAuth renewal qualification (test-only)", "Generated by npm run test:integration:oauth for an isolated short-lived Docker project.", [
  ...
  section("Provider settings qualification (test-only)", "Generated by npm run test:integration:providers for an isolated short-lived Docker project.", [
    ...
    variable("LEMMACOMPUTER_PROVIDER_PROBE_QUALIFICATION_MASTER_KEY", "generated-per-qualification", "Isolated callback-enabled provider-probe LiteLLM master key.", { secret: true }),
    variable("LEMMACOMPUTER_PROVIDER_PROBE_QUALIFICATION_SALT_KEY", "generated-per-qualification", "Isolated callback-enabled provider-probe LiteLLM salt key.", { secret: true }),
    variable("LEMMACOMPUTER_PROVIDER_PROBE_QUALIFICATION_POSTGRES_PASSWORD", "generated-per-qualification", "Isolated callback-enabled provider-probe PostgreSQL password.", { secret: true }),
    variable("LEMMACOMPUTER_PROVIDER_PROBE_QUALIFICATION_USAGE_TOKEN", "generated-per-qualification", "Isolated callback-enabled provider-probe usage signing token.", { secret: true }),
    variable("LEMMACOMPUTER_PROVIDER_PROBE_QUALIFICATION_LITELLM_PORT", "generated-per-qualification", "Isolated callback-enabled provider-probe LiteLLM host port."),
  ...
  section("Remote workspace-node qualification (test-only)", "Generated by npm run dev:remote-workspace for the current isolated worktree.", [
...
const variableGuidance = (item) => {
  ...
  if (item.generated) guidance.push("Local initialization: npm run env:init generates this value once in .env. Keep it with the installation data.");
...
export function renderEnvironmentTemplate() {
  const lines = [
    ...
    "#",
    "# Generated from scripts/setup/deployment-config.mjs. Do not edit this file by hand.",
    "# This is the full reference catalog, not a file to copy or fill out.",
    "# Run `npm run env:init` to create one compact .env with fresh local secrets.",
    "# Never commit .env. Use `npm run env:update` to compact an existing file.",
    "#",
    "# Scope: every deployment input, including optional overrides and generated secrets, is listed",
    "# exactly once below. Omitted settings use these defaults.",
    "# Service-local and per-workspace variables are derived",
...
export function resolveDeploymentEnvironment(input = {}) {
  ...
  const derived = raw.LEMMACOMPUTER_INSTALLATION_KIND === "worktree" && /^[a-f0-9]{10}$/.test(raw.LEMMACOMPUTER_INSTALLATION_ID ?? "")
    ? Object.fromEntries(worktreeEnvironmentOverrides({
      id: raw.LEMMACOMPUTER_INSTALLATION_ID,
      slug: `lemmacomputer-${raw.LEMMACOMPUTER_INSTALLATION_ID}`,
      portOffset: 1000 + (Number.parseInt(raw.LEMMACOMPUTER_INSTALLATION_ID.slice(0, 6), 16) % 20000),
    }))
    : {};
  const resolved = {};
  for (const item of environmentContract) {
    ...
    const fallback = derived[item.key] ?? item.default;
    resolved[item.key] = supplied === "" && fallback !== "" && !item.optional ? fallback : supplied ?? fallback;
  }
  return resolved;
}
...
export function validateDeploymentEnvironment(input = {}, { profile, strict = false } = {}) {
  ...
  if (hasValue(values.LEMMACOMPUTER_INSTALLATION_ID) && !/^[a-f0-9]{10}$/.test(values.LEMMACOMPUTER_INSTALLATION_ID)) {
    errors.push("LEMMACOMPUTER_INSTALLATION_ID must be a generated 10-character lowercase hexadecimal identifier");
  }
...
export function worktreeEnvironmentOverrides({ slug, id, portOffset }) {
  return new Map([
    ["LEMMACOMPUTER_INSTALLATION_ID", id],
...
const gatewayProviderEgressPolicy = '{"schemaVersion":2,"mode":"restricted","id":"egv_gateway_provider_egress_v2","securityGroupId":"esg_gateway_provider_egress","version":2,"name":"Gateway provider egress","description":"Reviewed model provider and catalog destinations.","defaultAction":"deny","rules":[{"id":"openai-api","action":"allow","protocol":"https","host":"api.openai.com","includeSubdomains":false,"port":443,"purpose":"OpenAI model API"},{"id":"anthropic-api","action":"allow","protocol":"https","host":"api.anthropic.com","includeSubdomains":false,"port":443,"purpose":"Anthropic model API"},{"id":"zai-api","action":"allow","protocol":"https","host":"api.z.ai","includeSubdomains":false,"port":443,"purpose":"Z.ai model API"},{"id":"bedrock-use1","action":"allow","protocol":"https","host":"bedrock-runtime.us-east-1.amazonaws.com","includeSubdomains":false,"port":443,"purpose":"Bedrock us-east-1"},{"id":"bedrock-usw2","action":"allow","protocol":"https","host":"bedrock-runtime.us-west-2.amazonaws.com","includeSubdomains":false,"port":443,"purpose":"Bedrock us-west-2"},{"id":"bedrock-euw1","action":"allow","protocol":"https","host":"bedrock-runtime.eu-west-1.amazonaws.com","includeSubdomains":false,"port":443,"purpose":"Bedrock eu-west-1"},{"id":"bedrock-apse1","action":"allow","protocol":"https","host":"bedrock-runtime.ap-southeast-1.amazonaws.com","includeSubdomains":false,"port":443,"purpose":"Bedrock ap-southeast-1"},{"id":"azure-openai","action":"allow","protocol":"https","host":"openai.azure.com","includeSubdomains":true,"port":443,"purpose":"Azure OpenAI resource APIs"},{"id":"azure-foundry","action":"allow","protocol":"https","host":"services.ai.azure.com","includeSubdomains":true,"port":443,"purpose":"Azure Foundry resource APIs"},{"id":"vertex","action":"allow","protocol":"https","host":"aiplatform.googleapis.com","includeSubdomains":false,"port":443,"purpose":"Vertex regional and global model APIs"},{"id":"google-token","action":"allow","protocol":"https","host":"oauth2.googleapis.com","includeSubdomains":false,"port":443,"purpose":"Google service account token exchange"},{"id":"litellm-catalog","action":"allow","protocol":"https","host":"raw.githubusercontent.com","includeSubdomains":false,"port":443,"purpose":"Public LiteLLM model metadata"},{"id":"vertex-us-central1","action":"allow","protocol":"https","host":"us-central1-aiplatform.googleapis.com","includeSubdomains":false,"port":443,"purpose":"Vertex us-central1 model API"},{"id":"vertex-us-east5","action":"allow","protocol":"https","host":"us-east5-aiplatform.googleapis.com","includeSubdomains":false,"port":443,"purpose":"Vertex us-east5 model API"},{"id":"vertex-europe-west1","action":"allow","protocol":"https","host":"europe-west1-aiplatform.googleapis.com","includeSubdomains":false,"port":443,"purpose":"Vertex europe-west1 model API"},{"id":"vertex-europe-west4","action":"allow","protocol":"https","host":"europe-west4-aiplatform.googleapis.com","includeSubdomains":false,"port":443,"purpose":"Vertex europe-west4 model API"},{"id":"vertex-asia-southeast1","action":"allow","protocol":"https","host":"asia-southeast1-aiplatform.googleapis.com","includeSubdomains":false,"port":443,"purpose":"Vertex asia-southeast1 model API"}],"documentHash":"fdf1166f9b36cbbaea5e19251517a6a08b2422ac31bcae717ffe42924361575b"}';
