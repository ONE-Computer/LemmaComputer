...
const variableGuidance = (item) => {
  ...
  if (item.generated) guidance.push("Local initialization: npm run env:init creates this value in persistent .env.state; back it up with .env.");
...
export function renderEnvironmentTemplate() {
  const lines = [
    ...
    "# Run `npm run env:init` to create a small .env and persistent .env.state.",
    "# Never commit either file. Use `npm run env:update` to compact an existing .env.",
    "#",
    "# Scope: every deployment input, including optional overrides and managed state, is listed",
