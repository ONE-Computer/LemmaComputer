...
const agentEnvironment = (
  ...
  return [
    ...
    `${prefix}_ALLOWED_SERVICE_CLASSES=${(policy.allowedServiceClasses ?? ["lite", "balanced", "pro"]).join(",")}`,
    ...
    const created = await this.createContainer(`/containers/create?name=${encodeURIComponent(name)}`, {
      ...
      Env: [
        ...
        ...(!runtimeInput.agentGrants && runtimeInput.gateway ? [
          ...
          `LEMMACOMPUTER_ALLOWED_SERVICE_CLASSES=${(input.policy.allowedServiceClasses ?? ["lite", "balanced", "pro"]).join(",")}`,
