...
This matrix is the reviewed contract for authentication and product-authorization data. Its machine-readable companion is [`tenant-isolation-manifest.json`](../../tests/contracts/tenant-isolation-manifest.json). The manifest names every persisted authentication and control-plane table created by the complete migration streams.
