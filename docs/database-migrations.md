...
Existing pre-ledger installations are not blindly marked current. The first runner transaction verifies the complete migration-028 table set, recent critical columns, removed legacy objects, and the `security_group_id NOT NULL` invariant. Only an exact compatible shape receives 28 `verified-legacy-baseline` ledger rows. Any discrepancy rolls back ledger creation and reports every observed mismatch.
...
- a raw legacy migration-028 schema is verified and baselined;
- an incompatible legacy schema is rejected without creating the ledger;
- changed historical SQL/checksum is rejected.
