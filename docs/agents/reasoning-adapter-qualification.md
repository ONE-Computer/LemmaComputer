...
2. Configure one supported authentication path for the isolated stack. The default `capture` email adapter provides a worktree-only browser link to its captured verification email, as described in the [single workflow guide](../guides/development-workflow.md#verify-a-local-account-without-external-email-delivery). For real external delivery, configure Postmark; a complete social or enterprise identity path is another option. Do not import an account from another stack.
