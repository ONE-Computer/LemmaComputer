...
| `KASM_LOCAL_KVM_ENABLED=true` | Legacy Claude Desktop **Computer** execution is enabled on a customer-managed host that exposes `/dev/kvm` and `/dev/vhost-vsock`; this is not the lightweight E2B Cowork path |
