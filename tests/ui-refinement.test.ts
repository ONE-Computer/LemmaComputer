...
test("workspace setup makes disposable-open an explicit accessible choice with durable lifecycle guidance", async () => {
  ...
  assert.match(app, /firewallAccessModelLabel\(settings\.securityGroup\?\.defaultAction\)/);
...
test("workspace options are editable, opt-in, and return to the overview with restart guidance after save", async () => {
  ...
  assert.match(app, /const protectedPolicyPlannedAgents = \["Codex Desktop"\]/);
...
test("the account menu gateways administrators to the AI control plane without adding primary navigation", async () => {
  ...
  assert.match(settingsScreen, /<strong>People and access<\/strong>/);
...
test("administration keeps identity in People and access while workspace operations remain contextual", async () => {
  ...
  assert.match(app, /Apply and restart compatible workspaces/);
  ...
  assert.match(app, /Network access follows workspace type/);
  assert.doesNotMatch(app, /title="Network access" lines=/);
  assert.match(app, /Per-workspace exceptions are managed in Network access/);
  assert.match(app, /const protectedPolicyReasoningOptions = \["low", "medium", "high"\]/);
...
test("Network access is an admin-only security-group library with type defaults and workspace overrides", async () => {
  ...
  assert.match(firewallScreen, /System defaults follow workspace type/);
  ...
  assert.match(firewallScreen, /Restricted workspace default/);
  assert.match(firewallScreen, /Internet workspace default/);
  assert.match(app, /ariaLabel="Network access model"/);
