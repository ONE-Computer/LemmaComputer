...
test("optional browser and agent artifacts are pinned and launch-gated", async () => {
  ...
  const codexConfig = await readFile("docker/workspace/lemmacomputer-codex-config.py", "utf8");
  assert.match(codexConfig, /model_provider = "lemmacomputer"/);
  assert.match(codexConfig, /base_url = "http:\/\/127\.0\.0\.1:4317\/v1"/);
