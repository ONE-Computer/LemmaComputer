...
test("administrator enters the AI control plane from the account menu and navigates its tabs", async ({ page }) => {
  ...
  await expect(explainability.getByText("Coming Soon", { exact: true })).toBeVisible();
  await expect(explainability.getByText("Why spend changed")).toHaveCount(0);
  await expect(explainability.getByText("Retry activity")).toHaveCount(0);
  await expect(explainability.getByText("Cache-read share")).toHaveCount(0);
