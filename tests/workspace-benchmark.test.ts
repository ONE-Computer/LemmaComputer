...
} from "./performance/workspace-benchmark-lib.mjs";
import {
  parseDockerStats,
  parseKasmCollectorArguments,
  webSocketPayloadBytes,
  workspaceBenchmarkEvent,
} from "./performance/collect-kasm-browser-runtime.mjs";
...
test("benchmark CLI can emit a deterministic metadata-only baseline without touching Docker", async () => {
  const { stdout } = await execute("node", [
    "tests/performance/benchmark-workspace.mjs",
  ...
  const cli = await readFile("tests/performance/benchmark-workspace.mjs", "utf8");
