...
spec = importlib.util.spec_from_file_location('identity', identity_module)
...
class TurnContextTests(unittest.TestCase):
        ...
        with tempfile.TemporaryDirectory() as home:
            ...
            try:
                ...
                while (
                    ...
                    and not h.gateway_process_matches_home(child.pid, home)
                ):
                    time.sleep(0.01)
    ...
    def test_auxiliary_headers_are_request_local_and_cannot_leave_broker(self):
        shared = {
            ...
            'extra_headers': {BH: 'stale', IH: 'stale', 'keep': 'yes'},
        }
        with h.bind_turn_context(context(A, 0)):
            for port in (4314, 4316):
                client = SimpleNamespace(base_url=f'http://127.0.0.1:{port}/v1/')
                result = h.auxiliary_request_overrides(client, shared)
        ...
        def run(identity):
            for turn in range(3):
                ...
                with h.bind_turn_context(ctx):
                    ...
                    self.assertEqual(
                        h.capture_agent_instance_meta(),
                        {'lemmacomputer': {'agentInstanceId': identity}},
                    )
                    barrier.wait(timeout=5)
    ...
    def test_explicit_api_context_masks_native_environment_and_stale_headers(self):
        with patch.dict(os.environ, {'LEMMACOMPUTER_AGENT_INSTANCE_ID': A}):
            ...
            with h.bind_turn_context(h.parse_turn_context({})):
                ...
                self.assertEqual(
                    h.model_request_overrides({
                        'extra_headers': {IH.upper(): A, BH: 'stale'},
                    }),
                    {'extra_headers': {}},
                )
            self.assertEqual(h.capture_agent_instance_meta()['lemmacomputer']['agentInstanceId'], A)
