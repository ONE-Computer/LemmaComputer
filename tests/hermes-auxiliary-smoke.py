...
try:
    with tempfile.TemporaryDirectory(prefix='hermes-auxiliary-') as home:
        ...
        runtime = {'provider':'custom','model':'lemmacomputer-balanced',
                   'base_url':'http://127.0.0.1:4314/v1','api_key':'fixture','api_mode':'chat_completions'}
        Path(home, 'config.yaml').write_text('model:\n  provider: custom\n  default: lemmacomputer-balanced\n  base_url: http://127.0.0.1:4314/v1\n  api_key: fixture\n')
        barrier = threading.Barrier(2)
        def run(index):
            runtime_for_turn = {**runtime, 'model': 'lemmacomputer-lite' if index == 1 else 'lemmacomputer-pro'}
            ...
            with bind_turn_context(TurnContext(binding,identity)):
                ...
                args = dict(provider='custom',model=runtime_for_turn['model'],base_url=runtime['base_url'],
                            api_key='fixture',main_runtime=runtime_for_turn,messages=[{'role':'user','content':str(index)}])
                ...
                maybe_auto_title(db,sid,f'Check arithmetic fixture {index}',main_runtime=runtime_for_turn)
                ...
                db.close()
                decision = _human_decision(MagicMock(), command='fixture', description='fixture',
                    pattern_key='fixture',pattern_keys=['fixture'],warnings=[],session_key=sid,
                    approval_callback=None,is_cli=False,is_gateway=True,is_ask=False,smart=False)
        ...
        for raw_headers, body in captured:
            ...
            counts[identity]+=1
            assert body['model']==('lemmacomputer-lite' if identity.startswith('1') else 'lemmacomputer-pro')
        ...
        print(json.dumps({'realAuxiliarySdkCalls':6,'concurrentIdentities':2,
              'backgroundTitleContextsPreserved':True,'nativeApprovalFailsClosedInWebChat':True}))
finally:
    server.shutdown()
