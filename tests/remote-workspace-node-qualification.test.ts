...
import {
  ...
  renderNodeCompose,
  replaceEnvironment,
} from "../scripts/development/remote-workspace-node.mjs";
...
test("qualification application forwarders authenticate node-local relays", async () => {
  const forwarder = await import("node:fs/promises").then(({ readFile }) => readFile(
    new URL("../scripts/development/remote-workspace-node-tls-forwarder.mjs", import.meta.url),
    "utf8",
  ));
...
test("qualification recovery scopes orphan-network cleanup to its Compose project", async () => {
  const source = await import("node:fs/promises").then(({ readFile }) => readFile(
    new URL("../scripts/development/remote-workspace-node.mjs", import.meta.url),
    "utf8",
  ));
