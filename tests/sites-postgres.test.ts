...
test("PostgreSQL Sites versions, grants, invitations, and tenant scoping are atomic", { skip: !connectionString }, async () => {
  ...
  try {
    ...
    assert.equal(await store.getSiteRole(orgAdmin, prepared.site), "viewer");
    assert.equal(await store.grantSiteAccess(orgAdmin, prepared.site.id, guestAccount), null);
    assert.equal((await store.deleteSite(orgAdmin, prepared.site.id)).deleted, false);
    const grant = await store.grantSiteAccess(owner, prepared.site.id, guestAccount);
    assert.equal(grant?.permission, "viewer");
    // An already-persisted Admin grant must retain reading, never management authority.
    await pool.query("UPDATE site_grants SET permission='admin' WHERE id=$1", [grant!.id]);
    assert.ok(await store.getAccessiblePublicationByHandle(external, prepared.site.handle));
    assert.equal((await store.listSiteGrants(owner, prepared.site.id))?.[0]?.permission, "viewer");
    assert.equal(await store.getSiteRole(external, prepared.site), "viewer");
    assert.equal((await store.deleteSite(external, prepared.site.id)).deleted, false);
    assert.equal(await store.grantSiteAccess(external, prepared.site.id, ownerAccount), null);
    assert.equal(await store.grantSiteAccess(owner, prepared.site.id, ownerAccount), null, "the owner cannot be downgraded via a grant");
    ...
    assert.equal(await store.listSiteGrants(external, prepared.site.id), null);
    assert.equal(await store.listSiteInvitations(external, prepared.site.id, new Date()), null);
    assert.equal(await store.listSiteVersions(external, prepared.site.id), null);
    assert.equal(await store.revokeSiteAccess(external, prepared.site.id, grant!.id), false);
    await store.revokeSiteAccess(owner, prepared.site.id, grant!.id);
