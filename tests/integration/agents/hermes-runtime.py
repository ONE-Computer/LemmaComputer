...
'''
    with tempfile.TemporaryDirectory(prefix='lemma-hermes-history-') as home:
        ...
        runtimes = [
            ...
            (sys.executable, candidate_source),
            (previous_python, previous_source),
        ]
        for phase, (python, source) in enumerate(runtimes):
        ...
        print(json.dumps({
            'syntheticHistoryUpgradeRollback': observations,
...
async def qualify():
    ...
    with ExitStack() as mocks:
        ...
        mocks.enter_context(patch.object(
            adapter,
            '_select_agent_runtime',
            side_effect=lambda runtime, model, **_: (model, None, None, None),
        ))
        mocks.enter_context(patch.object(adapter, '_ensure_session_db', return_value=MagicMock()))
        mocks.enter_context(patch.object(
            adapter,
            '_get_existing_session_or_404',
            new=AsyncMock(return_value=({'id': 'fixture'}, None)),
        ))
            ...
            for turn in range(2):
                async def request(index):
                    ...
                    response = await client.post(
                        ...
                        headers={binding_header: binding, identity_header: identities[index]},
                    )
                    ...
                    record = next(
                        ...
                        if row[0] == f'fixture-{index}'
                        and row[1]['extra_headers'].get(binding_header) == binding
                    )
                    assert record[1]['extra_headers'] == {
                        'fixture-header': 'preserved',
                        binding_header: binding,
                        identity_header: identities[index],
                    }
                    assert record[2] == {
                        'lemmacomputer': {'agentInstanceId': identities[index]},
                    }
                await asyncio.gather(request(0), request(1))
            malformed_headers = (
                {identity_header: identities[0]},
                {binding_header: 'a'*32 + '.b'},
                {identity_header: 'bad', binding_header: 'a'*32 + '.b'},
            )
            for headers in malformed_headers:
                response = await client.post(
                    '/api/sessions/fixture-0/chat',
                    json={'message': 'fixture'},
                    headers=headers,
                )
    ...
    async def list_tools(_):
        return web.json_response({'tools': [{
            ...
            'mcp_info': {
                'server_id': 'fixture',
                'server_name': 'lemmacomputer_exa',
            },
        }]})

    async def call_tool(request):
        headers = {key.lower(): value for key, value in request.headers.items()}
        received.append((headers, await request.json()))
    ...
    try:
        params = StdioServerParameters(
            ...
            args=[str(ROOT / 'docker/workspace/lemmacomputer-connectors-stdio.py')],
            env={
        ...
        async with stdio_client(params) as streams:
            async with ClientSession(*streams) as session:
                ...
                for identity in identities:
                    ...
                    result = await handlers._call_tool_racing_stdio_death(
                        server, 'fixture', name, {}, meta,
                    )
    ...
    print(json.dumps({
        ...
        'concurrentResumedHttpTurns': len(captured),
        'partialIdentityRejected': True,
        'failedToolEventPreserved': True,
        'mcpBridgeCalls': len(received),
