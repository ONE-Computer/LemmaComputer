...
with tempfile.TemporaryDirectory(prefix='hermes-lifecycle-') as root:
    ...
    base = {
        **os.environ,
        'HOME': str(root),
        'HERMES_DISABLE_BACKGROUND_REVIEW': '1',
    ...
    with (root / 'gateway.log').open('w') as log:
        ...
        try:
            ...
            while time.monotonic() < deadline and gateway.poll() is None and not healthy(port):
                time.sleep(.1)
            ...
            print(json.dumps({
                'realApiGateway': True,
                'realDesktopBackendStarts': 2,
                'separateHomesPreserved': True,
                'apiHealthAfterDesktopStartAndStop': True,
            }))
        finally:
            stop(gateway)
